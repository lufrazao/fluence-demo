...
export const PRODUCTS: Product[] = [
  ...
  { id: 4, name: 'Nespresso Vertuo', price: 549, originalPrice: 749, color: 'bg-amber-700', emoji: '☕', rating: 4.5, reviews: 876, category: 'Home', badge: 'Oferta', installment: '10x R$54,90' },
  ...
  { id: 6, name: 'Kindle Paperwhite', price: 549, originalPrice: 649, color: 'bg-emerald-700', emoji: '📱', rating: 4.8, reviews: 4231, category: 'Electronics', badge: 'Favorito', installment: '10x R$54,90' },
  { id: 7, name: 'Good Girl CH', price: 459, originalPrice: 599, color: 'bg-pink-600', emoji: '🌸', rating: 4.7, reviews: 1876, category: 'Beauty', badge: 'Oferta', installment: '10x R$45,90' },
...
const CATEGORIES = [
  { id: 'all', label: 'Todos' },
  { id: 'Electronics', label: '💻 Eletronicos' },
...
const PRICE_TERMS = ['barato', 'desconto', 'promocao', 'oferta', 'preco', 'custo', 'economico', 'parcela', 'frete gratis'];
const QUALITY_TERMS = ['melhor', 'premium', 'top', 'qualidade', 'original', 'profissional', 'nota', 'avaliacao'];
const IMPULSE_TERMS = ['rapido', 'agora', 'pronta entrega', 'urgente', 'hoje', 'imediato'];
const BRAND_NAMES = PRODUCTS.map(p => p.name.toLowerCase().split(' ')[0]); // sony, macbook, nike, etc.
...
const ShopHome: React.FC<ShopHomeProps> = ({ onNavigate, onEvent, cartCount, searchTrigger = 0 }) => {
  ...
  const handleProductClick = (product: Product) => {
    onEvent({
      type: 'product_click',
      label: `Viu ${product.name}`,
      detail: `R$${product.price.toLocaleString()} — produto`,
  ...
  const openSearch = useCallback(() => {
    ...
    onEvent({
      type: 'search_query',
      label: 'Abriu busca',
      detail: 'Abriu campo de busca',
      time: now(),
    });
    setTimeout(() => inputRef.current?.focus(), 100);
  }, [onEvent]);

  // Auto-open search when triggered from bottom nav
  const initialTrigger = useRef(searchTrigger);
  useEffect(() => {
    if (searchTrigger > initialTrigger.current) {
      openSearch();
    }
  }, [searchTrigger, openSearch]);
  ...
  const handleSearchInput = (value: string) => {
    ...
    // Debounce: fire search event after user pauses typing
    if (searchDebounce.current) clearTimeout(searchDebounce.current);
    if (value.trim().length >= 2) {
      searchDebounce.current = setTimeout(() => {
        const q = value.toLowerCase();

        // Detect intent from search terms
        const isPriceSearch = PRICE_TERMS.some(t => q.includes(t));
        const isQualitySearch = QUALITY_TERMS.some(t => q.includes(t));
        const isImpulseSearch = IMPULSE_TERMS.some(t => q.includes(t));
        const isBrandSearch = BRAND_NAMES.some(b => q.includes(b));

        // Find matched products to extract category context
        const matched = PRODUCTS.filter(p =>
          p.name.toLowerCase().includes(q) || p.category.toLowerCase().includes(q)
        );
        const isCategorySearch = CATEGORIES.some(c => c.id !== 'all' && q.includes(c.id.toLowerCase()));

        // Determine dominant matched category
        ...
        const avgPrice = matched.length > 0
          ? Math.round(matched.reduce((s, p) => s + p.price, 0) / matched.length)
          : undefined;

        // Build behavioral detail tags
        const tags: string[] = [];
        if (isBrandSearch) tags.push('busca especifica');
        if (isCategorySearch) tags.push('busca por categoria');
        if (isPriceSearch) tags.push('busca por preco');
        if (isQualitySearch) tags.push('busca por qualidade');
        if (isImpulseSearch) tags.push('busca impulsiva');
        if (matched.length === 0) tags.push('busca sem resultado');
        if (tags.length === 0 && matched.length > 0) tags.push('busca exploratoria');
        if (tags.length === 0) tags.push('busca generica');

        const detail = `"${value}" — ${tags.join(', ')}`;
    ...
    <div className="flex flex-col min-h-full bg-gray-50 pb-20">
      ...
      <div className="bg-gradient-to-r from-orange-500 to-orange-400 px-4 pt-2 pb-4">
        {searchMode ? (
          <div className="flex items-center gap-2">
            <div className="flex-1 flex items-center bg-white rounded-lg px-3 py-2 gap-2">
              ...
              />
              {searchQuery && (
                <button onClick={() => { setSearchQuery(''); inputRef.current?.focus(); }} className="text-gray-400 text-xs">
                  ✕
                </button>
              )}
            </div>
            <button onClick={closeSearch} className="text-white text-xs font-medium flex-shrink-0">
              Fechar
            </button>
          </div>
        ) : (
          <>
            <div className="flex items-center justify-between mb-3">
              <div>
                ...
                <p className="text-orange-100 text-[10px]">Descubra ofertas incriveis</p>
              </div>
              <button onClick={() => onNavigate('cart')} className="relative text-white">
            ...
            >
              ...
              <span className="text-white/60 text-xs">Buscar produtos...</span>
            </button>
          </>
        )}
      </div>
      ...
      {filtered.length > 0 ? (
        ...
        <div className="flex flex-col items-center justify-center px-6 py-12">
          ...
          <p className="text-[10px] text-gray-400 mt-1">Tente outro termo ou limpe a busca</p>
        </div>
      )}
    </div>
  );
