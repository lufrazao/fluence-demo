...
const CROSS_SELL = [
  { id: 20, name: 'Cabo USB-C', price: 29, emoji: '🔌' },
  { id: 21, name: 'Pelicula Protetora', price: 19, emoji: '🛡️' },
  { id: 22, name: 'Suporte Celular', price: 49, emoji: '📱' },
];
...
const CartScreen: React.FC<CartScreenProps> = ({ items, onNavigate, onEvent, onUpdateQty, onRemove }) => {
  ...
  const handleCheckout = () => {
    onEvent({
      ...
      detail: `R$${total.toLocaleString()} (${items.length} itens) — checkout`,
      time: now(),
    });
    onNavigate('checkout');
  };
  ...
  return (
    <div className="flex flex-col min-h-full bg-gray-50 pb-20">
      ...
      <div className="bg-gradient-to-r from-orange-500 to-orange-400 px-4 pt-2 pb-3 flex items-center gap-3">
        ...
        <h1 className="text-white font-semibold text-sm flex-1">Meu Carrinho</h1>
        <span className="text-white/80 text-xs">{items.reduce((s, i) => s + i.qty, 0)} itens</span>
      </div>

      {items.length === 0 ? (
        <div className="flex flex-col items-center justify-center flex-1 px-6 py-12">
          <span className="text-5xl mb-3">🛒</span>
          <p className="text-sm font-semibold text-gray-700">Seu carrinho esta vazio</p>
          <p className="text-xs text-gray-400 mt-1 text-center">Explore nossos produtos e encontre algo especial!</p>
          <button
        ...
        <div className="px-4 pt-3 space-y-3">
            ...
            {shippingRemaining > 0 ? (
              <>
                <p className="text-[10px] text-emerald-700 font-medium">
                  🚚 Adicione R${shippingRemaining.toLocaleString()} para FRETE GRATIS!
                </p>
          ...
          {/* Cross-sell */}
          <div>
            <p className="text-[10px] font-semibold text-gray-700 mb-2">✨ Complete seu Pedido</p>
          ...
          <div className="bg-white rounded-xl p-3 shadow-sm border border-gray-100 space-y-2">
            <p className="text-xs font-semibold text-gray-800">Resumo do Pedido</p>
            ...
            <div className="flex justify-between text-[10px] text-gray-500">
              ...
              <span className={subtotal >= FREE_SHIPPING_THRESHOLD ? 'text-green-600 font-medium' : ''}>
                {subtotal >= FREE_SHIPPING_THRESHOLD ? 'GRATIS' : 'R$15'}
              </span>
            </div>
