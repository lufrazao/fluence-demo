...
import type { MetricDef } from '../../types/simulation';
...
const MetricsCard: React.FC<MetricsCardProps> = ({ metrics, detectedPatterns = new Set() }) => {
  if (!metrics.length) return null;

  return (
    <div>
      <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2">Impacto Projetado no Negocio</h3>
            ...
            >
              ...
              {!unlocked && m.unlockedBy && (
                <div className="text-[10px] text-gray-600 mt-1">
                  {detectedPatterns.size > 0 ? '\u{1F512}' : '\u2014'}
                </div>
              )}
            </div>
          );
